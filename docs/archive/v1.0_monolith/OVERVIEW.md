...
2. Probe and capture fixtures (fresh <= 7 days or document a waiver).
...
- Freshness window is <= 7 days unless a waiver is documented.
...
- Fixture freshness is enforced (<= 7 days); missing or stale `captured_at` fails `scripts/fixture-audit.ts`.
...
- Spec: `MASTER_PLAN.md` (archived: `COLLABORATION_SPEC_ARCHIVED.md`)
