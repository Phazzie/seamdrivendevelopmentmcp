...
const FIXTURE_PATH = path.join(ROOT_DIR, "fixtures/notifications/sample.json");
const STORE_PATH = path.join(os.tmpdir(), "mcp_notif_probe_store.json");

async function run() {
  const jailedFs = new JailedFs(path.dirname(STORE_PATH));
  const store = new StoreAdapter(STORE_PATH, jailedFs);
  const notifs = new NotificationAdapter(store);

  await notifs.send({ title: "Alert", message: "Test" });
  const list = await notifs.list();
  ...
  console.log("Notifications fixture written.");
}
