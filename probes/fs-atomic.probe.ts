...
try {
  ...
  console.log("Initial file created. Inode:", initialStat.ino);
  ...
  fs.fsyncSync(fd);
  fs.closeSync(fd);
  console.log("Temp file fsync'd.");

  // 4. Atomic Rename
  fs.renameSync(TEMP_FILE, PROBE_FILE);
  const newStat = fs.statSync(PROBE_FILE);
  console.log("Rename complete. New Inode:", newStat.ino);
  ...
  if (initialStat.ino === newStat.ino) {
    ...
    fs.writeFileSync(path.join(FIXTURE_DIR, 'fs_capabilities.json'), JSON.stringify(fixture, null, 2));
  }
